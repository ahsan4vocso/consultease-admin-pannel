...
const statsController = ({ strapi: strapi2 }) => ({
  async getSummary(ctx) {
    try {
      ...
      const data = await strapi2.plugin("admin-pannel").service("stats").getSummaryStats(filters);
      ctx.send(data);
    } catch (error) {
...
const statsService = ({ strapi: strapi2 }) => ({
  ...
  async getPendingExpertStats() {
    const [tVer, tApp] = await Promise.all([
      ...
      strapi2.documents("api::expert-profile-approval.expert-profile-approval").count({ filters: { is_approved: false, expert_profile: { $notNull: true } } })
    ]);
  ...
  async getEconomyStats(range = {}) {
    try {
      ...
      strapi2.log.error("Economy Stats SQL Error:", error);
      return {
  ...
  async getGraphStats(interval = "day wise") {
    try {
      ...
      strapi2.log.error("Stats Service Error (Graph SQL):", error);
      throw error;
    }
  }
});
